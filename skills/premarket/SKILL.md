---
name: us-stock-premarket-brief
description: 평일 KST 21:30 미국 주식 장전 브리핑 (8:30 AM EDT) 생성 → kkangdago.com/us 배포 + 카카오톡 알림
---

매일 KST 21:30 (미국 EDT 08:30), 미국 주식 장전 브리핑을 생성하고 kkangdago.com/us에 배포합니다.
...
- 미국 EDT 기준 TODAY_US = KST 날짜 (KST 21:30 = 당일 EDT 08:30)
- YESTERDAY = TODAY_US - 1일

예시: KST 2025-06-14 21:30 → TODAY_US = 2025-06-14

## STEP 2: 뉴스 수집 (병렬)

NaverSearch-search_news로 다음 5개 쿼리 동시 실행 (display:10, sort:date):

1. "나스닥 S&P500 다우 마감 전망"
2. "미국 빅테크 엔비디아 애플 마이크로소프트 주가"
3. "미국 AI 반도체 테마 장전"
4. "연준 금리 달러 경제지표 오늘"
5. "뉴욕증시 선물 장전 시황"
...
수집된 뉴스를 바탕으로:
- **시장 요약**: 전일 마감 동향 + 오늘 장전 분위기 (3~5문장)
- **주목 테마 3~5개**: 실제 뉴스 기사에 근거한 테마명 + 한줄 설명
- **상승 유망 미국 종목 6개**:
  - 종목명 + 티커 심볼
  - 추천 근거 (뉴스 기반, 1~2문장)
  - 감성 점수 (긍정/중립/부정)
...
[INDEX_CARDS], [THEME_CARDS], [STOCK_CARDS], [RISK_CARDS], [NEWS_ITEMS]
를 실제 내용으로 채운다.
...
<head>
  ...
  <title>kkangdago US [TODAY_US] 장전 브리핑</title>
  <meta name="description" content="[TODAY_US] 미국 증시 장전 AI 브리핑. S&P500, 나스닥 주요 테마와 추천 종목.">
  ...
  <style>
    ...
    /* 전체 스타일 — kkangdago 한국 버전과 동일한 CSS variables 사용 */
    * { box-sizing:border-box; margin:0; padding:0; }
    ...
    .badge.premarket { color:var(--signal); border-color:#f0c04033; }
    ...
    .theme-grid,.stock-grid { display:grid; grid-template-columns:repeat(auto-fit,minmax(280px,1fr)); gap:1rem; }
    .theme-card { background:var(--surface); border:1px solid var(--border); border-left:3px solid var(--accent); border-radius:12px; padding:1.2rem 1.5rem; }
    .theme-name { font-weight:700; margin-bottom:.4rem; }
    .theme-desc { color:var(--text-muted); font-size:.9rem; }
...
<div class="container">
  ...
  <div style="margin-bottom:2rem;">
    <div style="display:flex;gap:.8rem;flex-wrap:wrap;align-items:center;margin-bottom:.8rem;">
      <span class="badge premarket" data-ko="장전 브리핑" data-en="Pre-Market">장전 브리핑</span>
      <span class="badge">AI Generated</span>
      <span style="color:var(--text-muted);font-size:.9rem;">[TODAY_US] · EDT 08:30</span>
    </div>
    <h1 class="brief-title" data-ko="[TODAY_US] 미국 증시 장전 브리핑" data-en="[TODAY_US] US Market Pre-Market Briefing">[TODAY_US] 미국 증시 장전 브리핑</h1>
  </div>

  <div class="section">
    <div class="section-title" data-ko="시장 요약" data-en="Market Summary">시장 요약</div>
  ...
  <div class="section">
    <div class="section-title" data-ko="주요 테마" data-en="Key Themes">주요 테마</div>
    <div class="theme-grid">[THEME_CARDS]</div>
  </div>

  <div class="section">
    <div class="section-title" data-ko="추천 종목" data-en="Stock Picks">추천 종목</div>
...
<div class="theme-card">
  <div class="theme-name" data-ko="[테마KO]" data-en="[테마EN]">[테마KO]</div>
  <div class="theme-desc" data-ko="[설명KO]" data-en="[설명EN]">[설명KO]</div>
</div>
```
...
<div class="stock-card">
  ...
  <div class="stock-reason" data-ko="[근거KO]" data-en="[근거EN]">[근거KO]</div>
...
### 5-1. archive/us-index.json SHA + 내용 읽기
```javascript
(async () => {
  const GH = 'Bearer [GH_TOKEN]';
  const r = await fetch('https://api.github.com/repos/worascal/kkangdago/contents/archive/us-index.json', {
...
### 5-2. us/index.html SHA 읽기 + 업데이트
```javascript
(async () => {
  const GH = 'Bearer [GH_TOKEN]';
  const REPO = 'worascal/kkangdago';
  const getR = await fetch('https://api.github.com/repos/' + REPO + '/contents/us/index.html', {
    headers: { 'Authorization': GH, 'Accept': 'application/vnd.github+json' }
  });
  const getJ = await getR.json();
  const content = '[HTML_CONTENT]';
  const r = await fetch('https://api.github.com/repos/' + REPO + '/contents/us/index.html', {
    method: 'PUT',
    headers: { 'Authorization': GH, 'Accept': 'application/vnd.github+json', 'Content-Type': 'application/json' },
    body: JSON.stringify({
      message: 'feat: [TODAY_US] US premarket briefing',
...
### 5-3. archive/us/[TODAY_US].html 저장
```javascript
(async () => {
  const GH = 'Bearer [GH_TOKEN]';
  const content = '[HTML_CONTENT]';
  const r = await fetch('https://api.github.com/repos/worascal/kkangdago/contents/archive/us/[TODAY_US].html', {
    method: 'PUT',
    headers: { 'Authorization': GH, 'Accept': 'application/vnd.github+json', 'Content-Type': 'application/json' },
    body: JSON.stringify({
      message: 'archive: US [TODAY_US] premarket',
      content: btoa(unescape(encodeURIComponent(content)))
    })
  });
  return { status: r.status };
...
### 5-4. archive/us-index.json 날짜 추가
기존 배열에 TODAY_US 추가 (중복 제거, 오름차순 정렬):
```javascript
(async () => {
  const GH = 'Bearer [GH_TOKEN]';
  // existingContent, existingSha는 5-1에서 획득
  const dates = JSON.parse(existingContent);
  if (!dates.includes('[TODAY_US]')) dates.push('[TODAY_US]');
  dates.sort();
  const newContent = JSON.stringify(dates, null, 2);
  const r = await fetch('https://api.github.com/repos/worascal/kkangdago/contents/archive/us-index.json', {
    method: 'PUT',
    headers: { 'Authorization': GH, 'Accept': 'application/vnd.github+json', 'Content-Type': 'application/json' },
    body: JSON.stringify({
      message: 'archive: update US index [TODAY_US]',
...
🔥 테마: [테마1], [테마2], [테마3]
⚡ 픽: [종목1]([TICKER1]), [종목2]([TICKER2]), [종목3]([TICKER3])
...
1. **뉴스 근거 없는 종목 추천 금지** — 수집된 뉴스에 언급된 종목만 추천
...
5. HTML 내 특수문자 이스케이프 주의 (역슬래시, 백틱 등)
